...
// This local component needs to import CommentView itself.
...
async fn fetch_comment_content(id: u32) -> Result<Comment, String> {
    ...
    let mut comment: Comment = reqwest::get(&url)
        .await
...
// A recursive rendering component for comments.
#[component]
fn RenderComments(
    comment_ids: Vec<u32>,
    ...
    rsx! {
        {
            comment_ids.iter().map(|id| {
                if let Some(comment) = comments_map.get(id) {
                    let children_nodes = if *comment.is_expanded.read() {
